...
assign(ENV, extend_environment(list(1, new SchemeDataType(SchemeDataType::TypeId::Symbol, "n")),  registers[ARGL], registers[ENV]));// (assign env (op extend-environment) (const (n)) (reg argl) (reg env))
s.save(registers[CONTINUE]);// (save continue)
s.save(registers[ENV]);// (save env)
assign(PROC, lookup_variable_value(new SchemeDataType(SchemeDataType::TypeId::Symbol, "="), registers[ENV])); // (assign proc (op lookup-variable-value) (const =) (reg env))
...
assign(VAL, lookup_variable_value(new SchemeDataType(SchemeDataType::TypeId::Symbol, "n"), registers[ENV]));// (assign val (op lookup-variable-value) (const n) (reg env))
...
assign(CONTINUE, new SchemeDataType(SchemeDataType::TypeId::String, "AFTER_CALL8"));// (assign continue (label after-call8))
...
assign(PROC, lookup_variable_value(new SchemeDataType(SchemeDataType::TypeId::Symbol, "factorial") , registers[ENV]));// (assign proc (op lookup-variable-value) (const factorial) (reg env))
s.save(PROC);// (save proc)
assign(PROC, lookup_variable_value(new SchemeDataType(SchemeDataType::TypeId::Symbol, "-"), registers[ENV]));// (assign proc (op lookup-variable-value) (const -) (reg env))
assign(VAL, new SchemeDataType(SchemeDataType::TypeId::Integer, 1));// (assign val (const 1))
assign(ARGL, list(registers[VAL]));// (assign argl (op list) (reg val))
...
assign(CONTINUE, new SchemeDataType(SchemeDataType::TypeId::String, "AFTER_CALL11"));// (assign continue (label after-call11))
assign(VAL, compiled_procedure_entry(registers[PROC])); // (assign val (op compiled-procedure-entry) (reg proc))
goto_with_label(registers[VAL]);// (goto (reg val))
primitive_branch9();
}
...
assign(CONTINUE, new SchemeDataType(SchemeDataType::TypeId::String, "AFTER_CALL14"));// (assign continue (label after-call14))
assign(VAL, compiled_procedure_entry(registers[PROC]));// (assign val (op compiled-procedure-entry) (reg proc))
goto_with_label(registers[VAL]);// (goto (reg val))
}
...
assign(VAL, apply_primitive_procedure(registers[PROC], registers[ARGL]));// (assign val (op apply-primitive-procedure) (reg proc) (reg argl))
return goto_with_label(registers[CONTINUE]);// (goto (reg continue))
after_call17();
}
...
define_variable(new SchemeDataType(SchemeDataType::TypeId::Symbol, "factorial"), registers[VAL], registers[ENV]);// (perform (op define-variable!) (const factorial) (reg val) (reg env))
