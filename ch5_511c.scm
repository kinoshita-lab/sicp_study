...
;; make-new-machineのstackのところを書きかえる(無駄にでかい)
(define (make-new-machine register-names) ;; regs名を引数に追加
  (let* ((pc (make-register 'pc))
        (flag (make-register 'flag))
		(stack (map (lambda (register-name)                    ;; ここがかわった
					   (cons register-name (make-stack)))
					 register-names))
		 ...
		 (list (list 'initialize-stack
					 (lambda ()
					   (for-each (lambda (stack)
								   (((cdr stack)) 'initialize) stack)))))) ;; ここがかわった全部初期化
          (register-table
      ...
      (define (dispatch message)
        ...
        (cond ((eq? message 'start)
              ...
              (else (error "UNKOUNKO Unknown request -- MACHINE" message))))
      dispatch))
...
;; make-save も変える必要あるし必要な関数もつくる
;; get-own-stackの試し用
(define (get-own-stack regname stacks)
  (if (eq? stacks '())
      '()
      (if (eq? regname (caar stacks))
          (cdar stacks)
          (get-own-stack regname (cdr stacks)))))

(define (make-save inst machine stacks pc) ;; stack -> stacks
  (let* ((reg (get-register machine
                           (stack-inst-reg-name inst)))
         (regname (stack-inst-reg-name inst)) ;; これと
         (my-stack (get-own-stack regname stacks))) ;; これ増えた
    (lambda ()
      (push my-stack (get-contents reg)) ;; 自分のstackを使うようにする
      (advance-pc pc))))

(define (make-restore inst machine stacks pc) ;; stack -> stacks
  (let* ((reg (get-register machine
                            (stack-inst-reg-name inst)))
         (regname (stack-inst-reg-name inst)) ;; これと
         (my-stack (get-own-stack regname stacks))) ;; これ増えた
