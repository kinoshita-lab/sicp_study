...
(load "./code_from_text/ch5-syntax.scm")			;section 4.1.2 syntax procedures
...
;;;SECTION 5.5.1
(define (compile exp target linkage compile-time-environment) ;; 足した
  (cond ((self-evaluating? exp)
         (compile-self-evaluating exp target linkage)) ;; self evaluatingはcompile環境いらなさそう
        ((quoted? exp) (compile-quoted exp target linkage)) ;; これもいらなそう
        ((variable? exp)
         (compile-variable exp target linkage compile-time-environment)) ;; これは要りそう
        ((assignment? exp)
         (compile-assignment exp target linkage compile-time-environment)) ;; これも要りそう
        ((definition? exp)
         (compile-definition exp target linkage compile-time-environment)) ;; これも要りそう
        ((if? exp) (compile-if exp target linkage))
        ((lambda? exp) (compile-lambda exp target linkage compile-time-environment)) ;; これすごい必要
        ((begin? exp)
         (compile-sequence (begin-actions exp)
                           target
                           linkage compile-time-environment)) ;; 中に必要なのが入ってそうだから必要
        ((cond? exp) (compile (cond->if exp) target linkage compile-time-environment))
        ((application? exp)
         (compile-application exp target linkage compile-time-environment)) ;; 必要
        (else
     ...
     (make-instruction-sequence '(env proc argl) '(env)
      `(,proc-entry
        ...
        (assign env
                ...
                (reg argl)
                (reg env))))
     (compile-sequence (lambda-body exp) 'val 'return (extend-compile-time-environment formals compile-time-environment))))) ;; 先へ
